...
import { getBlogList, getBlogContent } from '@/features/blog/api/blog.service';
import { groupBlogsByCategory } from '@/features/blog/utils/blog.utils';
import { logger } from '@/utils/logger';
import { ROUTES, BLOG_CONFIG } from '@/config';
...
export function useBlogTree(): UseBlogTreeReturn {
  ...
  const loadBlogContent = async (blogId: string) => {
    ...
    try {
      ...
      const blogMeta = blogList.find((b: BlogItem) => b.id === blogId);
      if (!blogMeta) {
  ...
  useEffect(() => {
    if (!id && categoriesInitializedRef.current && categories.length > 0) {
      ...
      const defaultBlogId = BLOG_CONFIG.defaultBlogId;
