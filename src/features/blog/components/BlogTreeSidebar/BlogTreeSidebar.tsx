...
import { LoadingLines } from '@/components/ui/LoadingLines/LoadingLines';
...
import { calculateTotalCount } from '@/features/blog/utils/blog.utils';
    ...
    <div className={`blog-tree-category blog-tree-category-level-${level}`}>
      ...
      <div className={`blog-tree-category-wrapper ${isExpanded ? 'expanded' : ''}`}>
        <div className={`blog-tree-category-content ${isExpanded ? 'expanded' : 'collapsed'}`}>
          {category.blogs.map((blog: { id: string; title: string }) => (
            <div
              key={blog.id}
          ...
          {hasChildren &&
            category.children?.map((child: BlogCategory) => (
